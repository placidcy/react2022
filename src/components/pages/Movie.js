...
function Movie(){
    ...
    const search = (query) => {
        var requestOptions = {
          ...
          fetch(`https://api.themoviedb.org/3/search/movie?api_key=${process.env.REACT_MOVIE_API_KEY}&query=${query}`, requestOptions)
    ...
    useEffect(() => { //값이 바뀔때마다 감지
        var requestOptions = {
          ...
          fetch(`https://api.themoviedb.org/3/search/movie?api_key=${process.env.REACT_MOVIE_API_KEY}&query=fear`, requestOptions)
            .then(response => response.json())
            .then(result => {
